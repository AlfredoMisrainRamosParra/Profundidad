	...
	public static void busqueda(String acomodado, pancake base, int vuelta) {
		for(int i = 2; i <= acomodado.length(); i ++) {
			...
			if(!ordenado) {
				...
				//En caso de que ya se encontro la solucion y vuelvo al caso raíz, reinicio algunas variables y cambio el limite a la solucion -1 para buscar una solucion optima
				if(vuelta == 1) {
					limite = caminoFinal(ultimoId).size()-1;
					ordenado = false;
					i = 2;
					cadenasYaHechas.clear();
					cadenasYaHechas.add(cadena);
				}
			}
		}
	}
